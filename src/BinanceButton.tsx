...
const INTEGRATION_ID = process.env.REACT_APP_MESH_INTEGRATION_ID ?? 'binance';
const NETWORK_ID = process.env.REACT_APP_MESH_BINANCE_NETWORK_ID;
const DEFAULT_SYMBOL = 'USDC';

const BinanceButton: React.FC<BinanceButtonProps> = ({ onDataUpdated, onRefreshAvailable }) => {
  ...
  const handleIntegrationConnected = React.useCallback(
    async (payload: LinkPayload) => {
      ...
      setConnectedInstitution(accessTokenPayload?.brokerName ?? 'Binance');
      setHoldings([]);
      setManagedAddress(null);
      setManagedAddressError(null);
      managedAddressRef.current = null;
      ...
      const integrationAccessToken: IntegrationAccessToken = {
        ...
        brokerType: accessTokenPayload?.brokerType ?? 'binance',
        brokerName: accessTokenPayload?.brokerName ?? 'Binance',
      };
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      try {
        localStorage.setItem('mesh-link-token', linkToken);
      } catch (storageError) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', position: 'relative' }}>
      <button
        ...
        disabled={isLinking}
      >
        <BinanceLogo style={{ width: 32, height: 32 }} />
        {isLinking ? 'Connecting…' : 'Connect to Binance'}
      </button>
      {linkError && (
        <div
          style={{
      ...
      {(connectedAccountLabel || holdingsError || isLoadingHoldings || holdings.length > 0 || managedAddressError || isLoadingAddress || managedAddress) && (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h3 style={{ margin: 0, fontSize: 20, color: '#15e6cd' }}>
            {connectedInstitution ?? 'Connected account'} holdings
          </h3>
          {authToken && (
            <button
